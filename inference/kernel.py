...
Lasmoid — kernel.py
===================
Custom Triton/TileLang kernels and pure-PyTorch fallbacks for:
...
def act_quant(
    ...
    """Block-wise FP8 activation quantisation with optional UE8M0 rounding."""
...
# ══════════════════════════════════════════════════════════════════════
# fp4_act_quant — FP4 block-wise quantisation
# ══════════════════════════════════════════════════════════════════════
def fp4_act_quant(
    x: torch.Tensor,
    block_size: int = 32,
    ...
    """Block-wise FP4 activation quantisation (simulated via nearest neighbor on MPS/CPU)."""
    assert x.is_contiguous(), "Input must be contiguous"
    if x.size(-1) < block_size:
        block_size = x.size(-1)
    else:
...
def weight_dequant(x: torch.Tensor, s: torch.Tensor, block_size: int = 128) -> torch.Tensor:
    """Dequantise FP8 weights using per-block scales."""
...
def fp8_gemm(
    ...
    """C[M,N] = A_fp8[M,K] @ B_fp8[N,K]^T with per-128-block FP8 scales."""
...
# ══════════════════════════════════════════════════════════════════════
# fp4_gemm — FP8 act × FP4 weight GEMM
# ══════════════════════════════════════════════════════════════════════
def fp4_gemm(
    a: torch.Tensor, a_s: torch.Tensor,
    b: torch.Tensor, b_s: torch.Tensor,
...
# ══════════════════════════════════════════════════════════════════════
# sparse_attn — Sparse attention via gathered indices
# ══════════════════════════════════════════════════════════════════════
def sparse_attn(
    q: torch.Tensor,            # (B, S, H, D)
    kv: torch.Tensor,           # (B, N, D)
    attn_sink: torch.Tensor,    # (H,)
    topk_idxs: torch.Tensor,    # (B, S, topk)
    softmax_scale: float,
    soft_cap: Optional[float] = None,
    pair_bias: Optional[torch.Tensor] = None, # (B, S, H, topk)
) -> torch.Tensor:
    """Sparse attention gathering only the top-k KV positions with optional relational pair bias."""
    B, S, H, D = q.shape
    topk = topk_idxs.shape[-1]
    
    # Bypass Triton kernel if pair_bias is provided to ensure correct PyTorch path execution
    if HAS_TRITON and SUPPORT_FP8_TRITON and q.is_cuda and pair_bias is None:
    ...
    idx_exp = safe_idx.unsqueeze(-1).expand(-1, -1, -1, D)
    ...
    pad_mask = ~valid_mask.unsqueeze(2).expand(-1, -1, H, -1)
...
# ══════════════════════════════════════════════════════════════════════
# hc_split_sinkhorn — hyper-connections Sinkhorn routing
# ══════════════════════════════════════════════════════════════════════
def hc_split_sinkhorn(
    mixes:         torch.Tensor,  # (B, S, (2+hc)*hc)
    ...
    """Decompose HC mixing logits into pre, post, and doubly-stochastic combination matrix."""
...
if __name__ == "__main__":
    print("=== Lasmoid Kernel Self-Test ===")
