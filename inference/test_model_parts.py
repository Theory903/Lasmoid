...
class TestLasmoidComponents(unittest.TestCase):
    ...
    def test_3_rotary_embeddings(self):
        ...
        freqs_cis = precompute_freqs_cis(self.args.rope_head_dim, 64, base=self.args.rope_theta).to(self.device)
        x = torch.randn(2, 10, self.args.n_heads, self.args.rope_head_dim, device=self.device)
    ...
    def test_6_compressor(self):
        ...
        compressor.freqs_cis = precompute_freqs_cis(self.args.rope_head_dim, 64, base=self.args.rope_theta).to(self.device)
    ...
    def test_7_compressed_attention_branches(self):
        ...
        csa = CompressedSparseAttention(self.args).to(self.device)
        hca = HeavilyCompressedAttention(self.args).to(self.device)
    ...
    def test_10_manifold_hyper_connections(self):
        ...
        mhc = ManifoldConstrainedHyperConnection(self.args.dim, self.args.num_residual_streams).to(self.device)
        
        x = torch.randn(2, 5, self.args.num_residual_streams, self.args.dim, device=self.device, requires_grad=True)
        A_l, B_l, C_l = mhc(x)
        
        self.assertEqual(A_l.shape, (2, 5, self.args.num_residual_streams, 1))
        self.assertEqual(B_l.shape, (2, 5, self.args.num_residual_streams, self.args.num_residual_streams))
    ...
    def test_11_mtp_block(self):
        ...
        h = torch.randn(2, 10, self.args.num_residual_streams, self.args.dim, device=self.device)
    ...
    def test_12_lasmoid_block(self):
        ...
        x = torch.randn(2, 10, self.args.num_residual_streams, self.args.dim, device=self.device)
        freqs_cis = precompute_freqs_cis(self.args.rope_head_dim, 64, base=self.args.rope_theta).to(self.device)
        
        out, z_loss, vq_loss, routing, indices, adj = block(x, freqs_cis[:10], start_pos=0, input_ids=torch.randint(0, self.args.vocab_size, (2, 10), device=self.device))
        self.assertEqual(out.shape, x.shape)
        self.assertEqual(z_loss.shape, ())
        self.assertEqual(vq_loss.shape, ())
    ...
    def test_13_lasmoid_full_model(self):
        ...
        total_concepts = self.args.num_concepts + self.args.num_abstract_concepts + self.args.num_global_concepts
    ...
    def test_14_loss_functions(self):
        ...
        loss_ce = compute_loss(
            logits=logits,
            targets=targets,
            routing_maps=[torch.randn(2, self.args.num_concepts, 10, device=self.device)],
            vq_losses=[torch.tensor(0.1, device=self.device)],
            adjacencies=[torch.randn(2, self.args.num_concepts, self.args.num_concepts, device=self.device)]
        )
